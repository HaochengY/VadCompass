...
class VLRefSegCore(nn.Module):
    ...
    forward(...):
      Inputs
        - video_embed: (B, N, L, D_video), N frames with L visual tokens per frame
      ...
      Returns
        ...
        - If forward_seg=True, adds seg_outputs:
            * frame_logits:   (B*K, 1, N)         # per-slot temporal logits
            * slot_frame_logits: (B, K, N)        # same logits before flattening slot dim
            * conf_logits:    (B, K)             # per-slot confidence logits
            * token_logits:   (B, K, N, H*W)      # per-slot per-frame visual-token logits
            * ref_vec:        (B, K, D_llm)
    """
    def __init__(
        self,
    ...
    def forward(
        ...
        # Call the LLM. Note: do NOT forward video_embed unless model expects it in kwargs.
    ...
    def _forward_seg(self, video_embed, ref_vec, sae_embeds, sae_attention_mask, unpadded_hw):
        ...
        seg_outputs = self.heatmap_head(
            video_embed=video_embed.to(dtype=hm_dtype, device=hm_device),
            ref_vec=ref_vec.to(dtype=hm_dtype, device=hm_device),
            querybook_slots=querybook_slots.to(dtype=hm_dtype, device=hm_device),
        )

        seg_outputs["ref_vec"] = ref_vec.to(device=hm_device, dtype=hm_dtype)

        return seg_outputs


class SlotAttnTemporalHead(nn.Module):
    """
    Multi-slot visual-token attention head with frame-level per-slot predictions.
    Inputs:
      - video_embed:    (B, N, L, D_video)
      - ref_vec:        (B, K, D_llm)
    ...
    Outputs:
      ...
      - slot_frame_logits: (B, K, N)        # same logits before flattening slot dim
      - conf_logits:    (B, K)             # per-slot confidence logits
      - token_logits:   (B, K, N, H*W)      # per-slot visual-token logits before spatial pooling
    """
    def __init__(
        self,
        llm_dim: int,
        ...
        super().__init__()
        video_dim = int(video_dim or llm_dim)
        assert video_dim % num_heads == 0
        self.video_dim = video_dim
        self.num_heads = num_heads
        self.d_head = video_dim // num_heads
        self.llm_dim = llm_dim
        self.querybook_dim = querybook_dim

        # projections
        self.q_proj = nn.Linear(llm_dim, num_heads * self.d_head, bias=False)
        self.k_proj = nn.Linear(video_dim, num_heads * self.d_head, bias=False)

        # head fusion over visual tokens
        if refine:
            ...
            self.fuser = nn.Linear(num_heads, 1)
    ...
    def forward(
        ...
        # shapes
        B, N, L, D = video_embed.shape
        assert D == self.video_dim, f"expected video_dim={self.video_dim}, got {D}"
        ...
        # dtype/dev alignment
        ref_vec = ref_vec.to(dtype=video_embed.dtype, device=video_embed.device)
        querybook_slots = querybook_slots.to(dtype=video_embed.dtype, device=video_embed.device)
        ...
        # attention logits per visual token, per frame
        Q = self.q_proj(fused_ref).view(B, K, self.num_heads, self.d_head)         # (B,K,Hh,Dh)
        Kmap = self.k_proj(video_embed).view(B, N, L, self.num_heads, self.d_head)
        Kmap = Kmap.permute(0, 1, 3, 4, 2).contiguous()                            # (B,N,Hh,Dh,L)
        logits_heads = (Q[:, :, None, :, :, None] * Kmap[:, None]).sum(dim=4) / math.sqrt(self.d_head)  # (B,K,N,Hh,L)
        token_logits = self.fuser(logits_heads.permute(0, 1, 2, 4, 3)).squeeze(-1)  # (B,K,N,L)

        # per-slot temporal prediction from spatial visual evidence + fused reference confidence
        token_pooled_logits = token_logits.amax(dim=-1)                            # (B,K,N)
        conf_logits = self.conf_head(fused_ref).squeeze(-1)                        # (B,K)
        return {
            "token_pooled_logits": token_pooled_logits,

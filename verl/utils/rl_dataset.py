...
class GetCollate:
    ...
    def collate_fn(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for key, value in tensors.items():
            if key not in ["pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw"]:
...
class RLHFDataset(Dataset):
    ...
    def _arrange_qwen_tokens(self, row_dict, prob_key):
        user_text = self.user_prompt.format(Question=row_dict[prob_key])
        user_text = user_text.replace("<image>", "").replace("<video>", "").strip()
        content = []
        for image in row_dict.get("images", []):
            content.append({"type": "image", "image": image})
        for video in row_dict.get("videos", []):
        ...
        if input_ids.size(0) > self.max_prompt_length:
        ...
        input_ids = pad_sequence_to_length(
            input_ids.unsqueeze(0), self.max_prompt_length, self.tokenizer.pad_token_id, left_pad=True
        ).squeeze(0)
        attention_mask = pad_sequence_to_length(
            attention_mask.unsqueeze(0), self.max_prompt_length, 0, left_pad=True
        ).squeeze(0)

        image_grid_thw = proc.get("image_grid_thw")
        video_grid_thw = proc.get("video_grid_thw")
        row_dict.update(
            {
                "pixel_values": proc.get("pixel_values"),
                "image_grid_thw": image_grid_thw,
                "pixel_values_videos": proc.get("pixel_values_videos"),
    ...
    def __getitem__(self, index):
        ...
        if "video" in row_dict:
            row_dict["videos"] = [row_dict["video"]]
    ...
    for b in range(B):
        ...
        if llm_version in ["qwen-2.5"]:
            position_id = get_rope_index(
                ...
                video_grid_thw=data.non_tensor_batch["video_grid_thw"][b]
                if "video_grid_thw" in data.non_tensor_batch
                else None,
                attention_mask=mask,
            )

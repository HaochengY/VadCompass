    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_episodes - self.curr_episode):
            for batch_dict in self.train_dataloader:
                ...
                if "pixel_values" in batch.non_tensor_batch.keys() or "pixel_values_videos" in batch.non_tensor_batch.keys():
                    vision_keys = [
                        key for key in [
                            "pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw",
                            "raw_prompt_ids", "raw_prompt", "images", "videos"
                        ]
                        if key in batch.non_tensor_batch.keys()
                    ]
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=vision_keys,
                    )
                else:
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],

...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if "pixel_values_videos" in micro_batch and micro_batch["pixel_values_videos"][0] is not None:
            vision_inputs["pixel_values_videos"] = torch.cat(micro_batch["pixel_values_videos"], dim=0)
            vision_inputs["video_grid_thw"] = torch.cat(micro_batch["video_grid_thw"], dim=0)

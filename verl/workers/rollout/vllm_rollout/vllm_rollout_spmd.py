...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "videos" in non_tensor_batch:
            vllm_inputs = []
            for raw_prompt, raw_prompt_ids, videos in zip(
                    non_tensor_batch.pop("raw_prompt"),
                    non_tensor_batch.pop("raw_prompt_ids"),
                    non_tensor_batch.pop("videos")
            ):
                if self.config.use_raw_prompt:
                    vllm_inputs.append({"prompt": raw_prompt, "multi_modal_data": {"video": videos}})
                else:
                    vllm_inputs.append({"prompt_token_ids": raw_prompt_ids, "multi_modal_data": {"video": videos}})
        elif "images" in non_tensor_batch:
            vllm_inputs = []
        ...
        if self.config.n > 1 and do_sample:
            ...
            if "pixel_values_videos" in non_tensor_batch.keys():
                non_tensor_batch["pixel_values_videos"] = _repeat_interleave(non_tensor_batch["pixel_values_videos"], self.config.n)
            if "video_grid_thw" in non_tensor_batch.keys():
                non_tensor_batch["video_grid_thw"] = _repeat_interleave(non_tensor_batch["video_grid_thw"], self.config.n)

...
class FSDPWorker(Worker):
    ...
    def compute_log_prob(self, data: DataProto):
        """
        Simultaneously compute the frame logits and masks for reward_fn
        """
        assert self._is_actor
        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)
        ...
        with self.ulysses_sharding_manager:
            ...
            log_prob, frame_logits, mask_sigmoid, conf_logits = self.actor.compute_log_prob(data=data, forward_seg=True)
            output = DataProto.from_dict(
                tensors={
                    "old_log_probs": log_prob,
                    "token_pooled_logits_detach": frame_logits,
                    "mask_sigmoid_detach": mask_sigmoid,
                    "conf_logits_detach": conf_logits,

...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        if "pixel_values" in micro_batch and micro_batch["pixel_values"][0] is not None:
        ...
        if "pixel_values_videos" in micro_batch and micro_batch["pixel_values_videos"][0] is not None:
            pixel_values_videos = torch.cat(micro_batch["pixel_values_videos"], dim=0)
            video_grid_thw = torch.cat(micro_batch["video_grid_thw"], dim=0) \
                if micro_batch["video_grid_thw"][0] is not None else None
            llm_inputs["pixel_values_videos"] = pixel_values_videos
            llm_inputs["video_grid_thw"] = video_grid_thw
            if forward_seg:
                sae_llm_inputs["pixel_values_videos"] = pixel_values_videos
                sae_llm_inputs["video_grid_thw"] = video_grid_thw
    ...
    def compute_log_prob(self, data: DataProto, forward_seg: bool = False) \
        ...
        Returns:
            ...
            if gen_mask is True:
                torch.Tensor: the log_prob tensor
                torch.Tensor: token_pooled_logits_detach
        ...
        log_probs_lst, frame_logits_lst, mask_sigmoid_lst, conf_logits_lst, = [], [], [], []
        ...
        return (
            log_probs,
            torch.concat(frame_logits_lst, dim=0),
            torch.concat(mask_sigmoid_lst, dim=0),
            torch.concat(conf_logits_lst, dim=0),
        )
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        for i, mini_batch in enumerate(mini_batches):
            ...
            for micro_batch in tqdm(micro_batches, desc=f"Update policy [{i + 1}/{n}]", disable=(self.rank != 0)):
                ...
                # compute frame-level BCE loss from temporal slot logits
                seg_loss, heatmap_bce, dice_loss, focal_loss, conf_loss = self._frame_bce_loss(
                    seg_outputs, model_inputs
                )
                ...
                loss = (
                    self.config.pg_loss_coef * policy_loss
                    + self.config.seg_loss_coef * seg_loss
                    + self.config.conf_loss_coef * conf_loss
                )
    ...
    def _frame_bce_loss(self, seg_outputs, model_inputs):
        logits = seg_outputs["token_pooled_logits"]
        target = self._frame_targets(model_inputs, logits)
        frame_bce = F.binary_cross_entropy_with_logits(logits, target, reduction="mean")
        conf_logits = seg_outputs["conf_logits"]
        conf_target = self._frame_conf_targets(logits, target, conf_logits)
        conf_loss = F.binary_cross_entropy_with_logits(conf_logits, conf_target, reduction="mean")
        zero = frame_bce.new_zeros(())
        return frame_bce, frame_bce, zero, zero, conf_loss

    def _frame_conf_targets(self, frame_logits, frame_targets, conf_logits):
        pred = torch.sigmoid(frame_logits.detach())
        gt = frame_targets.detach().clamp(0.0, 1.0)
        intersection = (pred * gt).sum(dim=-1)
        union = pred.sum(dim=-1) + gt.sum(dim=-1) - intersection
        soft_iou = intersection / union.clamp_min(1e-6)
        best_slot = soft_iou.argmax(dim=1)
        conf_target = torch.zeros_like(conf_logits)
        conf_target.scatter_(1, best_slot[:, None], 1.0)
        return conf_target

    def _frame_targets(self, model_inputs, logits):
        for key in ("frame_labels", "frame_label", "frame_targets", "labels", "label"):
            if key in model_inputs:
                target = model_inputs[key]
                break
        else:
            raise KeyError(
                "frame-level BCE requires one of frame_labels/frame_label/frame_targets/labels/label in model_inputs"
            )
        ...
        if target.dim() == 1:
            if target.numel() == N and B == 1:
                ...
                raise ValueError(f"cannot align frame target shape {tuple(target.shape)} to logits {(B, K, N)}")
        elif target.dim() == 2:
            if target.shape == (B, N):
            ...
            if target.shape == (B, 1, N):
                target = target.expand(B, K, N)
            elif target.shape != (B, K, N):
                raise ValueError(f"cannot align frame target shape {tuple(target.shape)} to logits {(B, K, N)}")
        else:
            raise ValueError(f"cannot align frame target shape {tuple(target.shape)} to logits {(B, K, N)}")
        return target
